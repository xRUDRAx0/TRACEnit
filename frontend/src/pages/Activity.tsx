...
export default function Activity() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12 pt-4 max-w-4xl mx-auto">
      ...
      {/* Feature 1: TRACE LIVE */}
      {isActive && (
        <div className="bg-surface rounded-xl shadow-lg border-2 border-accent/20 overflow-hidden mb-8">
          <div className="bg-surface-secondary p-4 flex items-center justify-between border-b border-border">
            <div className="flex items-center gap-3">
              <span className="relative flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-error opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-error"></span>
              </span>
              <h2 className="text-sm font-black text-text-primary uppercase tracking-widest">TRACE LIVE</h2>
            </div>
            <p className="text-[10px] font-bold text-text-muted uppercase">Observing workstation</p>
          </div>
          
          <div className="p-6 grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-text-muted uppercase">Recent Activity</h3>
                ...
                {liveEvents.slice(0, 5).map((act, i) => {
                  ...
                  return (
                    <div key={i} className="flex gap-3 items-start">
                      ...
                      <div>
                        <p className="text-xs font-bold text-text-primary capitalize">{getSemanticAction(act)}</p>
                        <p className="text-[10px] text-text-secondary truncate max-w-[200px]">{appName} {act.windowTitle ? `· ${act.windowTitle}` : ''}</p>
                      </div>
                    </div>
                  );
                })}
                {liveEvents.length === 0 && <p className="text-xs text-text-muted">Waiting for events...</p>}
              </div>
            </div>
            
            <div className="bg-surface-secondary/50 rounded-lg p-4 border border-border">
              <h3 className="text-xs font-bold text-text-muted uppercase mb-4">Pattern Intelligence</h3>
              {useObservation().livePattern && useObservation().livePattern.repeatedActions.length > 0 ? (
                <div className="space-y-3 animate-in fade-in zoom-in-95">
                  <div className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-warning" />
                    <p className="text-sm font-bold text-text-primary">Pattern Detected</p>
                  </div>
                  <p className="text-xs text-text-secondary">
                    {useObservation().livePattern.repeatedActions.join(' → ')}
                  </p>
                  <div className="pt-2 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-text-muted uppercase">Confidence</span>
                    <span className="text-xs font-black text-success">{useObservation().livePattern.workflowSimilarityPercentage}%</span>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-text-muted">Analyzing sequence...</p>
              )}
            </div>
          </div>
        </div>
